...
In order to get started with development a regular pc should be sufficient, we have an example using docker and python that will get you started easily. But docker or python is not at all a requirement for integrating the SDK.

- A browser capable of running javascript modules. Chrome, Firefox and Safari all work.
- A HD web camera connected to the pc.
...
### Retrieve a web sdk token

Before being able to use the 365id Id Verification SDK, you will need a _web sdk token token_ and to get that you will need to make a Rest API call with your client id and secret. If you don't have the credentials (clientid and client secret), please contact us at [info@365id.com](mailto:info@365id.com). Once you have received the credentials you will be able to request a _web sdk token_. Please note that the _web sdk token_ has a limited time to live.

**Url**: https://global-customer-frontend.365id.com

### Retrieve a web sdk token and a transaction id using the access token

The session token is what the SDK uses to tell the servers it's a valid connection attempt.

#### **/api/v1/websdktoken**

_Used for retrieving a new __web sdk token__ using the client id and secret, also known as customer external id and license key_
...
```json
{
  ...
  "transfer_device_domain_url": "string" // optional if want to continue on other devices
}
```
...
The _web sdk token_ is valid for a short amount of time.
...
> **⚠️ SECURITY NOTICE:** In a production solution, it is mandatory that you obtain the _web sdk token_ using a server-to-server call. Exposing the Client ID and Client Secret in the browser code is not allowed.
...
idVerification.start(
   "The token that you get through the web server", // Web SDK Token
   {
      ...
      onError: (exception) => {
         // Called when a error happens in the id verification process.
         verification.stop() // The SDK should be stopped after an error has been reported.
      },
      onClientConnectionLost: () => {
         // Called when the second device’s SDK connection is lost.
...
It is possible to set more options if that is needed. In the example below we do the following:
...
// Initialize the SDK.
idVerification.init(
   "id_of_div_you_want_the_sdk_to_operate_on",
   window.location.origin + "/public_file_path",
   initOptions
)
...
You are able to select the document size that you want the registered person to use. This option will alter the illustrations used throughout the id verification process.
...
The `show()` call is used to activate the verification process.
This will cause the SDK to render inside the supplied element identified by the id supplied in the `init()` call.
The SDK is designed to use the major part of the browser window, the supplied element should by that logic be allowed to expand to a major part of the browser window.
...
Before calling IdVerification365Id.start(), you can customize the SDK logo by using `IdVerification365Id.withTheme({logoUrl: "path/to/custom/logo"})`.
The theming possibilities are planned to be extended in later releases.
...
To implement the SDK inside your web app, we recommend an implementation that follows this diagram:
...
sequenceDiagram
   ...
   participant C as 365id Rest API
   participant A as 365id API
   ...
   B->>I: The Browser contacts your Web Server

   I->>C: Your server calls the websdktoken endpoint

   C->>I: return(transactionId, websdktoken)
   ...
   B->>S: Browser calls idverification.start() with websdktoken
   S->>B: return(true)
   ...
   I->>A: Your server calls the 365id Customer API to get the transaction result
   A->>I: return(TransactionResult)
...
For example, If we are unable to capture a clear image of the document the image capturing step will be retried.
...
```ts
onDocumentFeedback: (documentType: DocumentType, countryCode: string) {
...
```ts
onFaceMatchFeedbackWithSource: (faceMatchStatus: FaceMatchStatus, faceMatchSource: FaceMatchSource) {
